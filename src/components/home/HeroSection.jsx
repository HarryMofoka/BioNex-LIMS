    ...
    return (
        <div className="min-h-[calc(90vh-140px)] px-6 sm:px-12 lg:px-16 flex flex-col justify-between pb-12 relative z-10">
            <div className="flex flex-col lg:flex-row justify-between lg:items-end mt-8 gap-4">
                <div>
                    <h1 className="text-5xl lg:text-7xl text-white font-normal tracking-tight mb-4 text-glow drop-shadow-2xl leading-[1.1]">
            ...
            <div className="flex flex-col lg:flex-row flex-grow items-start lg:items-center gap-8 lg:gap-12 z-10 my-12">
                ...
                <div className="flex lg:flex-col gap-4">
                    <button
                        className={`font-mono text-xs tracking-widest transition-all duration-400 cubic-bezier w-20 h-20 lg:w-24 lg:h-24 rounded-[1.5rem] flex flex-col gap-2 items-center justify-center group ${activeTab === 'trace' ? 'active text-white bg-white/15 border-white/40 shadow-[0_0_30px_rgba(255,255,255,0.15)]' : 'text-white/40 hover:text-white bg-white/3 border-white/5 hover:bg-white/10 hover:border-white/20 hover:-translate-y-1 hover:shadow-[0_10px_30px_-5px_rgba(0,0,0,0.5)]'
                            }`}
                        onClick={() => setActiveTab('trace')}
                    >
                        <Icon icon="solar:box-linear" className="text-[32px] group-hover:scale-110 transition-transform duration-300" />
                        <span className="opacity-70">TRACK</span>
                    </button>
                    <button
                        className={`font-mono text-xs tracking-widest transition-all duration-400 cubic-bezier w-20 h-20 lg:w-24 lg:h-24 rounded-[1.5rem] flex flex-col gap-2 items-center justify-center group ${activeTab === 'evals' ? 'active text-white bg-white/15 border-white/40 shadow-[0_0_30px_rgba(255,255,255,0.15)]' : 'text-white/40 hover:text-white bg-white/3 border-white/5 hover:bg-white/10 hover:border-white/20 hover:-translate-y-1 hover:shadow-[0_10px_30px_-5px_rgba(0,0,0,0.5)]'
                            }`}
                        onClick={() => setActiveTab('evals')}
                    >
                        <Icon icon="solar:clipboard-list-linear" className="text-[32px] group-hover:scale-110 transition-transform duration-300" />
                        <span className="opacity-70">AUDIT</span>
                    </button>
                    <button
                        className={`font-mono text-xs tracking-widest transition-all duration-400 cubic-bezier w-20 h-20 lg:w-24 lg:h-24 rounded-[1.5rem] flex flex-col gap-2 items-center justify-center group ${activeTab === 'demo' ? 'active text-white bg-white/15 border-white/40 shadow-[0_0_30px_rgba(255,255,255,0.15)]' : 'text-white/40 hover:text-white bg-white/3 border-white/5 hover:bg-white/10 hover:border-white/20 hover:-translate-y-1 hover:shadow-[0_10px_30px_-5px_rgba(0,0,0,0.5)]'
                            }`}
                        onClick={() => setActiveTab('demo')}
                    >
                        <Icon icon="solar:widget-5-linear" className="text-[32px] group-hover:scale-110 transition-transform duration-300" />
            ...
            <div className="flex items-end justify-between z-10 pt-4 border-t border-white/5">
                <div className="w-full md:w-1/2 bg-black/20 h-20 rounded-2xl border border-white/10 flex items-center justify-between px-6 relative overflow-hidden group cursor-pointer backdrop-blur-sm">
                    ...
                    <div className="flex items-center gap-4 z-10">
                        <Icon icon="solar:api-linear" className="text-white/60 text-2xl" />
                        <div className="flex flex-col">
                            <span className="font-mono text-xs text-white/40 mb-1">CONNECT INSTRUMENTS</span>
                            <span className="font-mono text-lg text-white/90">nexus.api.connect()</span>
                        </div>
                    </div>
